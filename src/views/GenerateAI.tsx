...
export default function GenerateAI() {
  ...
  const generateRecipie = useAppStore((state) => state.generateRecipie);
  const recipie = useAppStore((state) => state.recipie);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (prompt.trim() === "") {
      showNotification({
        text: "La busqueda no puede ser vacia",
        error: true,
      });
      return;
    }

    await generateRecipie(prompt);
  };
  return (
    <>
      ...
      <div className="max-w-4xl mx-auto">
        <form onSubmit={handleSubmit} className="flex flex-col space-y-3 py-10">
          <div className="relative">
            ...
            <button
              type="submit"
              aria-label="Enviar"
              className={`cursor-pointer absolute top-1/2 right-5 transform 
        ...
        <div className="py-10 whitespace-pre-wrap">{recipie}</div>
      </div>
    </>
  );
}
